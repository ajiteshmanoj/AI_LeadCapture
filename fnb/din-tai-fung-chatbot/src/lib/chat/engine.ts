...
import { parseActions, executeBookTable, executeAddToWaitlist } from "./actions";
...
export async function handleChat(req: ChatRequest): Promise<ChatResponse> {
  ...
  if (conversation.status === "escalated" || conversation.status === "closed") {
    ...
    const ack =
      conversation.status === "escalated"
        ...
        : "This chat has been closed. If you need anything, please contact us directly at the outlet.";
  ...
  if (isOptOut(req.message)) {
    ...
    const optOutReply =
      "Got it — I've stopped this chat. Your details won't be used further for reservation purposes. Feel free to reach out anytime.";
  ...
  const { cleanReply, bookTable, waitlist } = parseActions(rawReply);
  ...
  if (bookTable) {
    const result = await executeBookTable(req.org_id, conversation.id, bookTable);
    if (!result.ok) {
      finalReply = `${finalReply}\n\n(Sorry, I had trouble saving that reservation — ${result.error}. Please call us at ${org.phone ?? "our outlet"} so we can sort it out.)`;
    } else {
      booked = true;
      const followups: string[] = [];

      if (result.invitedAttendee && bookTable.contact_email) {
        followups.push(
          `Calendar invite sent to ${bookTable.contact_email} — accept it to add the reservation to your calendar.`,
        );
      }

      // Web/SMS bookers: surface opt-in links so they can receive reminders.
  ...
  if (waitlist) {
    ...
    if (!result.ok) {
      finalReply = `${finalReply}\n\n(Sorry, I had trouble adding you to the waitlist — ${result.error}. Please call us at ${org.phone ?? "the outlet"}.)`;
    } else {
