...
//
// Each restaurant admin completes their own OAuth consent. We store the
// resulting refresh token on the organisation row and use it to create
// calendar events when reservation bookings happen via chat.
...
> {
  ...
  // subject may be a full calendar title (e.g. "Reservation: 4 pax — Birthday @ Outlet")
  // or a short subject label. Use it as-is if it already contains a dash/colon, otherwise wrap.
  const summary = /[—:\-]/.test(input.subject)
    ? input.subject
    : `${input.subject} — ${input.studentName} (${input.level})`;
  const descriptionLines = [
    `Guest: ${input.studentName}`,
    input.parentName ? `Contact: ${input.parentName}` : null,
    `Phone: ${input.parentPhone}`,
    input.parentEmail ? `Email: ${input.parentEmail}` : null,
    `Details: ${input.subject}  |  Party: ${input.level}`,
    input.teacherName ? `Note: ${input.teacherName}` : null,
    "",
    "(Auto-created from Din Tai Fung reservation chatbot)",
  ].filter(Boolean);
