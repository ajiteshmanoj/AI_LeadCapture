  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Used by the chatbot to answer guest questions.</p>
      </div>
      <div className="grid grid-cols-1 gap-4">
        <Field label="Restaurant name *"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Din Tai Fung @ Paragon" /></Field>
        <Field label="Address"><Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="290 Orchard Rd, #B1-03 Paragon" /></Field>
        <Field label="Phone"><Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+65 6836 8336" /></Field>
        <Field label="Email"><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="reservations@dintaifung.com.sg" /></Field>
      </div>
