...
export function ClassManager({
  ...
  async function remove(c: ClassRow) {
    if (!confirm(`Delete reservation slot: ${c.subject}?`)) return;
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <Field label="Time Slot / Occasion">
          <Input
            value={form.subject}
            onChange={(e) => setForm({ ...form, subject: e.target.value })}
            placeholder="Lunch Service"
          />
        </Field>
        ...
        <div className="flex items-end">
          <Button onClick={add} disabled={busy} className="w-full">
            {busy ? "Adding…" : "Add reservation slot"}
          </Button>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 border-b">
            <tr className="text-left">
              <th className="px-3 py-2 font-medium">Time Slot / Occasion</th>
              <th className="px-3 py-2 font-medium">Max Party Size</th>
            ...
            {initialClasses.length === 0 && (
              <tr>
                <td colSpan={9} className="px-3 py-8 text-center text-muted-foreground">
                  No reservation slots yet — add one above.
                </td>
              </tr>
            )}
          </tbody>
