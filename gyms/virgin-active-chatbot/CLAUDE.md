...
**Virgin Active Singapore chatbot** — a gym-vertical fork of the tuition-chatbot
codebase, adapted for fitness clubs. Bot name: **Alex**. Primary colour: `#E3000F`.

The codebase retains all tuition-chatbot DB column names unchanged (the schema is
shared; only UI labels and the chat logic are gym-branded). The mapping is:

| DB column / tuition term | Virgin Active display label |
|--------------------------|-----------------------------|
...
| `subject`                | Class Name                  |
| `level`                  | Fitness Level               |
| `locations` / Centre     | Club                        |
| `booking_type: trial`    | Trial Class                 |
| `monthly_fee`            | Monthly Fee                 |
| `registration_fee`       | Joining Fee                 |
| `[BOOK_TRIAL]` action    | `[BOOK_TRIAL_CLASS]`        |
| `[ADD_TO_WAITLIST]`      | `[JOIN_WAITLIST]`           |

Gym-specific additions (migration 011): `class_category`, `fitness_level`,
`class_duration_minutes`, `is_virtual` on `classes`; `membership_tier`,
`membership_expiry`, `fitness_goals` on `students`; `membership_tiers` table.

Full feature roadmap with phase plan lives at `FEATURE_ROADMAP.md`.
