...
export default function CentreStep() {
  ...
  async function next() {
    if (!name.trim()) { setError("Gym/Club name is required."); return; }
    setBusy(true);
    setError(null);
  ...
  return (
    <div className="space-y-5">
      <div>
        <h2 className="text-lg font-semibold">Gym/Club details</h2>
        <p className="text-sm text-muted-foreground">Used by the chatbot to answer member questions.</p>
      </div>
      <div className="grid grid-cols-1 gap-4">
        <Field label="Gym/Club name *"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Virgin Active Novena" /></Field>
        <Field label="Address"><Input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="50 Jurong Gateway Rd, #03-21 JEM" /></Field>
        <Field label="Phone"><Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+65 6789 0001" /></Field>
        <Field label="Email"><Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="enquiries@zenith.edu.sg" /></Field>
      </div>
