...
export async function handleChat(req: ChatRequest): Promise<ChatResponse> {
  ...
  if (conversation.status === "escalated" || conversation.status === "closed") {
    ...
    const ack =
      conversation.status === "escalated"
        ...
        : "This chat has been closed. If you need anything, please reach out to your nearest Virgin Active club directly.";
  ...
  const { cleanReply, bookTrial, waitlist, membershipEnquiry } = parseActions(rawReply);
  ...
  if (bookTrial) {
    ...
    if (!result.ok) {
      ...
      const attendeeEmail = bookTrial.contact_email ?? bookTrial.parent_email;
      if (result.invitedAttendee && attendeeEmail) {
        followups.push(
          `📧 Calendar invite sent to ${attendeeEmail} — accept it to add the trial class to your calendar.`,
        );
      }
  ...
  if (waitlist) {
    ...
    if (!result.ok) {
      finalReply = `${finalReply}\n\n(Sorry, I had trouble adding you to the waitlist — ${result.error}. Please call us at ${org.phone ?? "the club"}.)`;
    } else {
      waitlistPosition = result.position ?? null;
    }
  }

  // Best-effort membership enquiry notification to admin
  if (membershipEnquiry) {
    const tierLabel = membershipEnquiry.tier ?? "unspecified";
    const name = membershipEnquiry.member_name ?? "Unknown";
    const phone = membershipEnquiry.contact_phone ?? "—";
    notifyAdmin(
      req.org_id,
      [
        "🏋️ *Membership enquiry*",
        `Name: ${name}`,
        `Phone: ${phone}`,
        `Tier interest: ${tierLabel}`,
        appUrlOrPlaceholder(`/dashboard/conversations/${conversation.id}`),
      ].join("\n"),
    ).catch((err) => console.warn("[notifyAdmin membership]", err));
  }
