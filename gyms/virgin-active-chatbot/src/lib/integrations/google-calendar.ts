...
export interface CreateBookingEventInput {
  ...
  /** Override the calendar event title. Defaults to "Trial: <name> — <subject> (<level>)" */
...
> {
  ...
  const descriptionLines = [
    ...
    input.teacherName ? `Teacher: ${input.teacherName}` : null,
    "",
    "(Auto-created from chatbot trial booking)",
  ].filter(Boolean);
