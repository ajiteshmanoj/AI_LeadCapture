...
A running log of work sessions. Newest at the top.
Each session: short bullet list of what changed and why.

---

## 2026-05-10 — Virgin Active gym fork: Steps 2–15

- Forked from tuition-chatbot codebase into `gyms/virgin-active-chatbot/`.
- `package.json` name updated to `virgin-active-chatbot`.
- UI label changes (display only, DB unchanged): students→Members, teacher_name→Instructor, Subject→Class Name, Level→Fitness Level, Centre→Club, Bookings→Class Bookings, trial→trial class, monthly_fee→Monthly Fee, registration_fee→Joining Fee.
- Sidebar icon: GraduationCap→Dumbbell; app title→"Virgin Active Chatbot"; branding strip→"Virgin Active".
- Migrations `011_gym_schema.sql` (gym columns + membership_tiers table) and `012_seed_virgin_active.sql` (org, 3 clubs, 5 classes, 3 tiers, 5 FAQs) added.
- Types updated: new gym fields on ClassRow/Student/Booking; MembershipTier interface; trial_class/class_booking/membership intents added.
- `intents.ts` rewritten for gym domain (trial_class, class_booking, membership, fees, schedule, complaint).
- `actions.ts` rewritten: `[BOOK_TRIAL_CLASS]` / `[JOIN_WAITLIST]` / `[ENQUIRE_MEMBERSHIP]` action tags; new payload shape (`member_name`, `contact_phone`) with legacy aliases kept.
- `prompts.ts` rewritten: Alex persona, trial-first rule, freeze policy redirect, membership tier copy, gym booking flow.
- `google-calendar.ts`: accepts optional `calendarTitle` override; calendar event title now `` `Trial Class: ${memberName} — ${className}` ``.
- Admin notification: booking ping changed to "💪 *New trial class booking*".
- `engine.ts`: handles new `membershipEnquiry` action; membership enquiry fires Telegram admin ping; text updated (parent→member, centre→club).
- Widget: guard `window.__GYM_CHATBOT_LOADED`, storage key `vag_conversation_id`, PDPA "for class booking purposes", defaults Alex/#E3000F.
- `knowledge_base/` created with 5 files (overview, classes, clubs, membership, faq) ready to upload via Documents dashboard.
- CLAUDE.md updated with domain-mapping table (DB column ↔ VA label).
- README.md rewritten for gym context.
- `.env.example` updated with gym-specific comments.
