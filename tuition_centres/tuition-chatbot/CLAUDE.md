...
Multi-tenant AI chatbot for Singapore tuition centres. Sold as a productised
service. Tiered pricing: **Starter $80/mo** (chat + trial booking + web
widget), **Growth $150/mo** (+ messaging + lead nurture + payment + lesson
reminders), **Pro $250/mo** (+ makeup booking + progress routing + broadcasts
+ referrals + NPS). One-time setup fee $400-600 covers onboarding + document
upload + customisation. One Postgres database, one Next.js app — every
business table is scoped by `org_id`. Full feature roadmap with phase plan
lives at `FEATURE_ROADMAP.md` (the source of truth for what to build next).
