...
export default function CentreStep() {
  ...
  async function next() {
    if (!name.trim()) { setError("Centre name is required."); return; }
    setBusy(true);
    setError(null);
  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Used by the chatbot to answer parent questions.</p>
      </div>
      <div className="grid grid-cols-1 gap-4">
        <Field label="Centre name *"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Zenith Education Studio" /></Field>
