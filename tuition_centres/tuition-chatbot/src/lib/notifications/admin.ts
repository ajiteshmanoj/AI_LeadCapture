...
export async function notifyAdmin(
  ...
  if (!org?.admin_telegram_chat_id) {
    console.warn("[notifyAdmin] skipped — admin_telegram_chat_id is null for org", orgId, "(link admin via /linkadmin in Settings)");
    return { ok: false, reason: "Admin not linked" };
  }
  if (!org.telegram_bot_token) {
    console.warn("[notifyAdmin] skipped — no telegram_bot_token for org", orgId);
...
export function formatBookingNotification(args: {
  ...
  return [
    ...
    `${args.bookingDate} ${time} @ ${venue}`,
    `Parent: ${parent}${args.parentEmail ? ` / ${args.parentEmail}` : ""}`,
  ].join("\n");
}
