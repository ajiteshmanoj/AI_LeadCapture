...
const STORAGE_KEY = "tcw_conversation_id";
const PDPA_NOTICE =
  "By chatting, you agree to our data collection for enquiry purposes. Type STOP anytime to opt out.";
...
export function renderWidget(config: WidgetConfig) {
  ...
  async function send() {
    ...
    // Create a live bot bubble that tokens stream into directly —
    // much faster than waiting for the full response before showing anything.
    const liveBubble = document.createElement("div");
    liveBubble.className = "tcw-msg tcw-msg-bot tcw-msg-streaming";
    liveBubble.innerHTML =
      '<span class="tcw-dot"></span><span class="tcw-dot"></span><span class="tcw-dot"></span>';
    ...
    await sendMessageStream(
      ...
      },
      {
        onToken(token) {
          ...
          accumulated += token;
          liveBubble.textContent = accumulated;
          messagesEl.scrollTop = messagesEl.scrollHeight;
        },
        onDone(cid, followup) {
          conversationId = cid;
