...
import type { ClassRow, FAQ, Location, PropertyListing } from "@/types";
...
export type PropertyListingRow = PropertyListing & {
...
export async function loadActiveListings(
  ...
  if (error) {
    console.error("[rag] loadActiveListings error", error);
    return [];
  }
  return (data ?? []) as unknown as PropertyListingRow[];
}
