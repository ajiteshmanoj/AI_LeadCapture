...
//
// Each Huttons office admin completes their own OAuth consent. We store the
// resulting refresh token on the organisation row and use it to create
// calendar events when viewing bookings happen via chat.
...
> {
  ...
  // If subject already begins with "Viewing:" (property vertical), use it as-is.
  // Otherwise fall back to the tuition format.
  const summary = input.subject.startsWith("Viewing:")
    ? input.subject
    : `Trial: ${input.studentName} — ${input.subject} (${input.level})`;
  const descriptionLines = [
    `Lead/Student: ${input.studentName}`,
    input.parentName ? `Name: ${input.parentName}` : null,
    `Phone: ${input.parentPhone}`,
    input.parentEmail ? `Email: ${input.parentEmail}` : null,
    `Property: ${input.subject}  |  Budget/Level: ${input.level}`,
    input.teacherName ? `Agent: ${input.teacherName}` : null,
    "",
    "(Auto-created from Huttons chatbot viewing booking)",
  ].filter(Boolean);
