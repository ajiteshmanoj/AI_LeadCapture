...
//
// The Huttons admin links their Telegram chat to the org once via the
// /linkadmin command. After that, anything noteworthy in the system (new
// viewing booking, pipeline lead, escalation) pings them in Telegram.
//
...
export function formatBookingNotification(args: {
  ...
  return [
    ...
    `Lead: ${parent}${args.parentEmail ? ` / ${args.parentEmail}` : ""}`,
  ].join("\n");
}
