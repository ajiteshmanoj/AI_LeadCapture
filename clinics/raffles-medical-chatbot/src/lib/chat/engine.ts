...
import { parseActions, executeBookAppointment, executeAddToWaitlist } from "./actions";
...
export async function handleChat(req: ChatRequest): Promise<ChatResponse> {
  ...
  if (conversation.status === "escalated" || conversation.status === "closed") {
    ...
    const ack =
      conversation.status === "escalated"
        ...
        : "This chat has been closed. If you need anything, please reach out to the clinic directly.";
    ...
    if (conversation.status === "escalated") {
      notifyAdmin(
        req.org_id,
        [
          "💬 *Patient replied during takeover*",
          `Channel: ${req.channel}`,
  ...
  const { cleanReply, bookAppointment, waitlist } = parseActions(rawReply);
  ...
  if (bookAppointment) {
    const result = await executeBookAppointment(req.org_id, conversation.id, bookAppointment);
    if (!result.ok) {
      finalReply = `${finalReply}\n\n(Sorry, I had trouble saving that booking — ${result.error}. Could you call us at ${org.phone ?? "the clinic"} so we can sort it out?)`;
    } else {
      booked = true;
      const followups: string[] = [];

      if (result.invitedAttendee && bookAppointment.contact_email) {
        followups.push(
          `📧 Calendar invite sent to ${bookAppointment.contact_email} — accept it to add the appointment to your calendar.`,
        );
      }

      // Web bookers have no messaging connection yet. Surface deep links
      // to opt into Telegram so they can receive reminders.
